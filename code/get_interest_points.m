...
gaus1 = fspecial('gaussian',9,2);
gaus2 = fspecial('gaussian',3,1);

filtered_img = imfilter(image,gaus2);
[gradx, grady] = imgradientxy(filtered_img);
...
gradxy = gradx .* grady;
gradxy = imfilter(gradxy, gaus1); 
gradxx = gradx.*gradx; 
gradxx = imfilter(gradxx, gaus1); 
gradyy = grady.*grady;
gradyy = imfilter(gradyy, gaus1); 


%Harris Value Calculation
h = gradxx .* gradyy - gradxy.^2 - alfa .*(gradxx + gradyy) .* (gradxx + gradyy);


%Add border to keep interest points or we can remove these corner points
border = zeros(s);
border(descriptor_window_image_width+1 : end - descriptor_window_image_width, descriptor_window_image_width +1: end - descriptor_window_image_width) = 1;
